...
function WWP.CreatePrebuffWindow()
	...
	for i = 1, 5 do
		...
		local editBox = WINDOW_MANAGER:CreateControlFromVirtual( prebuffBox:GetName() .. "EditBox", prebuffBox,
			"ZO_DefaultEdit" )
		...
		local checkBox = WINDOW_MANAGER:CreateControlFromVirtual( prebuffBox:GetName() .. "CheckBox", prebuffBox,
			"ZO_CheckButton" )
        ...
        ClearCursor()
				...
				if previousAbilityId and previousAbilityId > 0 then
					...
					local skillType, skillLine, skillIndex = GetSpecificSkillAbilityKeysByAbilityId( previousProgression
						:GetAbilityId() )
